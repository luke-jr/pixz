...
*-v*::
  Verbose listing. When used with *-l* in tarball mode, prints four columns per
  file: the compressed file offset of the containing LZMA block, the uncompressed
  byte offset of the file within that block, the total uncompressed size of that
  block, and the filename. Format: `block_compressed_offset uncompressed_offset_in_block block_uncompressed_size filename`.
  To decompress a specific file: seek to `block_compressed_offset` in the archive,
  decompress the block, and skip `uncompressed_offset_in_block` bytes.
